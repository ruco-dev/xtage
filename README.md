...
### 0.6.0
- **Moved to the `@ruco-dev` scope.** Install with `npx -y @ruco-dev/xtage` / `claude mcp add xtage npx -- -y @ruco-dev/xtage`. The scope now matches the project's own domain. `@ruco-ai/xtage` is deprecated — not removed — so existing installs keep working.
- **Canonical home** is now [xtage.ruco.dev](https://xtage.ruco.dev).
